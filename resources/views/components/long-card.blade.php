...
@if ($link != 'false') 
    <div {{ $attributes->merge(['class' => "flex space-x-5 bg-white/10 box-border p-3 rounded-lg border border-transparent hover:border-blue-700 w-full group"])}}>
        ...
        <div class="flex-1 flex flex-col justify-center">
            ...
            <a href="{{$link}}" class="font-bold text-xl group-hover:text-blue-400">{{ $job->title }}</a>
            <a href="{{$link}}" class="text-sm text-gray-400">{{ $job->schedule }}: {{ $job->salary }}</a>
        </div>
...
@else
    ...
    <div {{ $attributes->merge(['class' => "flex space-x-5 bg-white/10 box-border p-3 rounded-lg border border-transparent hover:border-blue-700 w-full group"])}}>
        ...
        <div class="flex-1 flex flex-col justify-center">
            <p class="text-sm text-gray-400">{{ $job->employer->name }}</p>
            <a href="{{$job->url}}" target="_blank" class="font-bold text-xl group-hover:text-blue-400">{{ $job->title }}</a>
            <a href="{{$job->url}}" target="_blank"  class="text-sm text-gray-400">{{ $job->schedule }}: {{ $job->salary }}</a>
        </div>
        <div class="flex items-start space-x-1 text-xs">
